$(document).ready(function(){ 
 ...
 /* var table_companies = $('#table_companies').dataTable({
    ...
    "columns": [
      ...
      { "data": "contact_number","sClass": "integer" },
      { "data": "location" },
 ...
 /* jQuery.validator.setDefaults({
  ...
  function show_message(message_text, message_type){
    $('#message').html('<h3 style="color:black"><b>' + message_text + '</b></h3>').attr('class', message_type);
	 ...
	 $("#form_company").trigger("reset");
      request.done(function(output){
        if (output.result == 'success')
		{
            ...
            show_message("Registered successfully", 'success');
          /* Reload datable
          table_companies.api().ajax.reload(function(){
            hide_loading_message();
            var company_name = $('#company_name').val();
            show_message(" '" + name + "' registered successfully.", 'success');
          }, true);*/
        } else {
          hide_loading_message();
          show_message(' Registration Failed', 'error');
        }
      });
      request.fail(function(jqXHR, textStatus){
        hide_loading_message();
        show_message('Registration Failed: ' + textStatus, 'error');
      });
   //}
  });
