$(document).ready(function(){
  ...
  var table_companies = $('#table_companies').dataTable({
    ...
    "columns": [
      ...
      {"data": "last_name"},
      { "data": "login_name","sClass": "login_name" },
      { "data": "password","sClass": "password" },
      { "data": "email_id","sClass": "email_id" },
      { "data": "user_role","sClass": "integer" },
      { "data": "contact_number" },
	  { "data": "address"},
  ...
  $(document).on('click', '#add_company', function(e){
   ...
   // $('#form_company #id').val('');
    ...
    $('#form_company #login_name').val('');
    $('#form_company #password').val('');
    $('#form_company #email_id').val('');
    $('#form_company #user_role').val('');
  ...
  $(document).on('click', '.function_edit a', function(e){
    ...
    request.done(function(output){
      if (output.result == 'success'){
        ...
        $('#form_company #login_name').val(output.data[0].login_name);
        $('#form_company #password').val(output.data[0].password);
        $('#form_company #email_id').val(output.data[0].email_id);
        $('#form_company #user_role').val(output.data[0].user_role);
